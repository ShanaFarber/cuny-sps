...
xml_table$copies_cold_worldwide <- as.integer(xml_table$copies_cold_worldwide)
...
The data from HTML and JSON read in exactly the same. For the XML, I needed to cast the numeric columns to integer type, as they read in as characters. I also for some reason cannot get my XML `copies_sold_worldwide` column to format correctly in `kable()`. Other than, the table are identical. 
