...
```{r load-txt}
raw <- readLines("C:/Users/Shoshana/Documents/CUNY SPS/cuny-sps/DATA 607/project1/player_stats.txt")
...
str_view_all(raw[-c(1,2,3,4)], '\\w+\\s\\w+(\\s\\w+)?(\\s\\w+)?')

# check for total score
str_view_all(raw, '\\d\\.\\d')

# check for pre_post_ratings
str_view(raw[-c(1,2,3,4)], '\\d+(P\\d+)?(\\s+)?\\->(\\s+)?\\d+(P\\d+)?')

str_view(raw[-c(1,2,3,4)], '\\d+(P\\d+)?(\\s+)?\\->')
...
player_names <- unlist(str_extract_all(raw[-c(1,2,3,4)], '\\w+\\s\\w+(\\s\\w+)?(\\s\\w+)?'))

# totals are the only numbers with a decimal
totals <- unlist(str_extract_all(raw, '\\d\\.\\d'))

# ratings in the format of number -> number
pre_post_ratings <- unlist(str_extract_all(raw[-c(1,2,3,4)], '\\d+(P\\d+)?(\\s+)?\\->(\\s+)?\\d+(P\\d+)?'))

pre_ratings <- str_remove(unlist(str_extract_all(unlist(str_extract_all(pre_post_ratings, '\\d+(P\\d+)?(\\s+)?\\->')), '\\d+(P\\d+)?')), 'P\\d+')
...
states <- unlist(str_extract_all(unlist(str_extract_all(raw[-c(1,2,3,4)], '[A-Z][A-Z]\\s\\|')), '[A-Z][A-Z]'))
```
...
```{r rounds-info}
# each round has either W, L, D with a number, or H, U, X
rounds <- unlist(str_extract_all(unlist((str_extract_all(str_remove(raw, '^\\s+[A-Z].+'), '\\|[WLBDXUH]\\s+(\\d+)?'))), '[WLBDXUH]\\s+(\\d+)?'))

# create a seven column matrix from rounds to have a data frame of the results of each round for each player
rounds <- matrix(rounds, byrow=T, ncol=7)
```
...
full_stats <- full_stats %>%
  filter(str_detect(full_stats$results, '\\w+\\s+\\d+')) # only keep rows where the round has an opponent listed
...
```{r}
pre_ratings <- data.frame("opp_player_id" = c(1:64),
                          "opp_pre_rating" = chess_stats$pre_rating)

avg_opp_pre_ratings <- full_stats %>%
  left_join(pre_ratings, on = "opp_player_id") %>%
...
```{r}
write.csv(chess_stats, 'C:/Users/Shoshana/Documents/CUNY SPS/cuny-sps/DATA 607/chess_stats.csv', row.names = FALSE)
```
