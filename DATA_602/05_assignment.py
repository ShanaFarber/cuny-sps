...
def exercise08():
    ...
    # ------ Place code below here \/ \/ \/ ------
    data = pd.read_csv('https://raw.githubusercontent.com/data602sps/assignments/master/Sacramentorealestatetransactions.csv')
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    # test failed for 2n x 2n so I did an nxn
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    dates = pd.date_range(start='01/01/2010', periods=n, freq='D')
    s = pd.Series(np.random.randint(0, n+1, n), index=dates)
