...
70% of adults in my sample think climate change affects their local community. 
...
I would not expect another student's sample to be identical to mine because it is highly unlikely that they set the same seed as I did. I would expect it to be similar though, as the sample is coming from the same population. 
...
"95% confidence" means that we are 95% confident that the true population proportion falls within this interval.
...
The interval calculated here is 58.3%-80%. We know the true proportion to be 62% which does fall within the interval. Thus, our confidence interval does capture the true value.
...
I would expect 95% of these intervals to capture the true proportion. This is because the interval is constructed for 95% confidence so I would expect about 95% of intervals to actually capture the population proportion. 
...
```{r plot-95-ci, echo=FALSE, results="asis", out.width='100%'}
knitr::include_graphics("C:/Users/Shoshana/Documents/CUNY SPS/cuny-sps/DATA_606/confidence_interval.png")
```

From the plot, 48/50 = 96% of intervals include the population proportion. This is close to, but not exactly, our confidence interval. The slight difference could be due to the sampling variability. So, while we expect for 95% of the intervals to contain the true value, in practice this value may not be exact. However, it is close enough. (It is also worth to note that, for 50 confidence intervals, you cannot get exactly 95% of the intervals, as that would be 47.5 intervals and we cannot have half intervals). 
...
_**90% CI**_: I would expect this interval to be narrower than the 95% CI, as we are less confident that the true proportion is contained within.

_**99% CI**_: I would expect this interval to be wider than the 95% CI, as we are more confident that the true proportion is contained within. 

### Exercise 8

**Using code from the _infer_ package and data from the one sample you have (`samp`), find a confidence interval for the proportion of US Adults who think climate change is affecting their local community with a confidence level of your choosing (other than 95%) and interpret it.**
...
The 90% confidence interval for this sample is 60%-80%. This means we are 90% confident the true proportion falls within this range. 

### Exercise 9

**Using the app, calculate 50 confidence intervals at the confidence level you chose in the previous question, and plot all intervals on one plot, and calculate the proportion of intervals that include the true population proportion. How does this percentage compare to the confidence level selected for the intervals?**

```{r plot-90-ci, echo=FALSE, results="asis", out.width='100%'}
knitr::include_graphics("C:/Users/Shoshana/Documents/CUNY SPS/cuny-sps/DATA_606/confidence_interval90.png")
```

41/50 = 82% of the intervals contain the true proportion for the population. This percentage is less than the 90% confidence level selected. 

### Exercise 10

**Lastly, try one more (different) confidence level. First, state how you expect the width of this interval to compare to previous ones you calculated. Then, calculate the bounds of the interval using the _infer_ package and data from `samp` and interpret it. Finally, use the app to generate many intervals and calculate the proportion of intervals that capture the true population proportion.**

I would expect the width of this interval to be larger than that of the previous 90% confidence interval.
...
The 99% confidence interval for this sample is 53%-83%. We are 99% confident the true population proportion will fall within this range.  

```{r plot-99-ci, echo=FALSE, results="asis", out.width='100%'}
knitr::include_graphics("C:/Users/Shoshana/Documents/CUNY SPS/cuny-sps/DATA_606/confidence_interval99.png")
```

49/50 = 98% of intervals contain the true proportion for the population. 
...
The width of the intervals decrease as sample size increases. This is because the standard error decreases with the increase in sample size. 
...
Increasing the number of bootstraps does not appear to have an effect on the width of the interval. 
