...
If we are interested in estimating the proportion of people who don't think the work scientists do benefits them, we can use the `sample_n` command to survey the population.
...
The distribution of this sample is pretty similar to the population, with 74% saying they believe the work scientists do benefits them and 26% saying they don't believe it benefits. Because it is a sample, the estimate for "Benefits" is slightly lower and the estimate for "Doesn't benefit" is slightly higher but it is still a close estimate and almost representative of the actual population. 

Depending on the sample that is taken, the estimated will change slightly but should still be similar to the actual population distribution. 
...
Since the seed was pre-set for this lab in the global options (`set.seed(1234)`), I would think that the results would be the same for everyone in the class. If someone did not realize the seed was already set though and they set a new seed, or if they decided to change the seed from the one that was already set, then the results would not be exactly the same as mine. I would expect that, since the sample is taken from the same population, the distribution would be similar and yet somewhat different. We can see this below with a new sampling. 
...
The proportion of people who don't believe the work scientists do benefits them is 10% lower for `samp2` than `samp1`. 

If we were to take another two sample of size 100 and size 1000, I would think the larger sample size would be a more accurate estimate. This is because a larger sample size is closer to the size of the population and so the distribution would probably be a bit closer to the reality than a smaller sample size. 
...
```{r plot-sampling-dist, fig.show='hold'}
...
There are 15,000 elements in `sample_props50`. The sampling distribution is normal with a center at 0.2 (20%). 

### Exercise 5

**To make sure you understand how sampling distributions are built, and exactly what the `rep_sample_n` function does, try modifying the code to create a sampling distribution of _25 sample proportions_ from _samples of size 10_, and put them in a data frame named `sample_props_small`. Print the output. How many observations are there in this object called `sample_props_small`? What does each observation represent?**
...
There are 24 observations in `the distribution`sample_props_small (there is no listed observation for replica 12), each representing one sample distribution of size 10 (i.e. 10 people). 
...
**What does each observation in the sampling distribution represent? How does the mean, standard error, and shape of the sampling distribution change as the sample size increases? How (if at all) do these values change if you increase the number of simulations? (You do not need to include plots in your answer.)**

Each observation represents the proportion from a sample of the defined size (10, 50, or 100). As sample size increases, the mean gets closer to the true value and the standard error decreases. Once the mean matches the true value, it does not change, but the standard error still decrease. The shape of the distribution becomes more centralized and less spread as sample size increases (meaning the distribution gets narrower and taller as the sample distribution more accurately reflects the true distribution and there are more observations in the center of the distribution). 

The values do not change when you increase the number of simulations. 
...
```{r}
samp15 <- sample_n(global_monitor, size = 15)
...
The best estimate based on this sample is that 93.3% of people think that the work scientists do does benefit them. 
...
```{r sample-15, fig.show='hold', out.width='80%', fig.align='center'}
...
The distribution is slightly left skewed and very spread, with the most observations at 0.8 and 0.85. I would estimate that 80-85% of people think the work scientists do enhances their lives. 
...
The population proportion is 80%. 
...
```{r sample-150, fig.show='hold', out.width='80%', fig.align='center'}
...
The shape is more normal and less spread. I would guess the proportion to be 80%. 
...
The sampling distribution for the sample of 150 has a smaller spread. If you are concerned with the estimate, you would prefer to have a smaller spread. 